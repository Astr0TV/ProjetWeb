...
public class ConnexionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if(u!=null && u.getEtat()!=false && u.getType().equals("admin") ) {
                response.sendRedirect("Admin");
                request.getSession(true).setAttribute("user", u);
            }
            if (u!=null && u.getEtat()!=false && u.getType().equals("conseiller") ) {
                response.sendRedirect("Conseiller");
                request.getSession(true).setAttribute("user", u);
                
            }
            if (u!=null && u.getEtat()!=false) {
                response.sendRedirect("ClientHome");
                request.getSession(true).setAttribute("user", u);
                
            }
            else{
                request.setAttribute("msg", "votre compte n'a pas encore obtenu l'accès");
                request.getRequestDispatcher("index.jsp").forward(request, response);
            }
        } catch (Exception e) {
