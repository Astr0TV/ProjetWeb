 ...
 * @author Huawei
 */
@WebServlet(name = "ShoppingSerlvet", urlPatterns = {"/Shopping"})
public class ShoppingSerlvet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet ShoppingSerlvet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet ShoppingSerlvet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user != null) {
            try {
                int id = user.getId();
                List<Compte> compes = CompteDao.getSoldeShopping(id);
                request.setAttribute("compes", compes);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user != null) {
            try {
                ...
                CompteDao.Shopping(prix, nucarte);
                request.setAttribute("idperson", idperson);
                HistoriqueDao.InsertionOperations(prix, nucarte, idperson);
